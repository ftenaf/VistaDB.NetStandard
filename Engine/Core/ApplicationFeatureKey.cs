    ...
    public class ApplicationFeatureKey
    {
      ...
      'I'
        ...
        internal ApplicationFeatureKey(byte[] rawPayload)
        {
            ...
            m_RawPayload = new byte[15];
            for (int index = 0; index < 15; ++index)
                m_RawPayload[index] = rawPayload[index];
            if (!HasValidChecksum)
                throw new ArgumentException("Payload checksum is not valid.");
            m_Key = RawToString(ShaCfbCipher(m_RawPayload, true));
        }
        ...
        public byte[] RawPayload
        {
            get
            {
                return (byte[])m_RawPayload.Clone();
            }
        }
        ...
        public bool IsProduction
        {
            get
            {
                return (KeyFlags & LicenseKeyFlags.Production) != LicenseKeyFlags.None;
            }
        }
        ...
        public bool IsTrial
        {
            get
            {
                return (KeyFlags & LicenseKeyFlags.Trial) != LicenseKeyFlags.None;
            }
        }
        ...
        public bool IsExpiring
        {
            get
            {
                return (KeyFlags & LicenseKeyFlags.Expiring) != LicenseKeyFlags.None;
            }
        }
        ...
        public byte[] UniqueValue
        {
            get
            {
                ...
                for (int index = 0; index < 8; ++index)
                    numArray[index] = m_RawPayload[index];
                return numArray;
            }
        }
        ...
        public bool HasValidChecksum
        {
            get
            {
                return ValidPayload(m_RawPayload);
            }
        }
        ...
        public static bool IsValidKey(string key)
        {
            if (key == null)
                return false;
            string serialNumber = NormalizeKey(key);
            if (string.IsNullOrEmpty(serialNumber) || serialNumber.Length != 25)
                return false;
            if (serialNumber.IndexOfAny(IllegalCharacters) >= 0)
                return false;
            try
            {
        ...
        public static string FormatKey(string rawKey)
        {
            if (string.IsNullOrEmpty(rawKey))
                return string.Empty;
            string str1 = NormalizeKey(rawKey);
            string str2;
            if (str1.Length < 5)
            {
                str2 = str1;
        ...
        private LicenseKeyFlags KeyFlags
        {
            get
            {
                return (LicenseKeyFlags)m_RawPayload[11];
            }
        }
        ...
        private static byte[] StringToRaw(string key)
        {
            ...
            if (key.Length < 24 || key.Length > 25)
                throw new ArgumentOutOfRangeException(nameof(key), "Serial number must be of length 25.");
            if (key.Length == 25 && !IsValidKey(key))
        ...
        private static long StringToChunk(string chunkString)
        {
            ...
            long num = 0;
            for (int index = 0; index < 8; ++index)
                num = num << 5 | CharToBits(chunkString[index]);
            return num;
        }
        ...
        private static char ComputeChecksum(string serialNumber)
        {
            ...
            {
                long num2 = num1 + CharToBits(serialNumber[index]);
                num1 = (num2 >> 5) + (num2 & 31L);
            }
            return BitsToChar((num1 >> 5) + (num1 & 31L));
        }
        ...
        internal static string ArrayToBase32(byte[] array)
        {
            ...
            while (index < num2)
            {
                ...
                if (index % 5 == 0)
                {
                    stringBuilder.Append(ChunkToString(chunk));
                    chunk = 0L;
                }
            }
            return stringBuilder.ToString();
