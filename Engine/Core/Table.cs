...
{
    internal class Table : Dictionary<string, Index>, ITable, IVistaDBTable, IDisposable
    {
        ...
        private Index activeOrder;
        private Index pkOrder;
        private Status rowStatus;
        private ContextStack context;
        private int clones;
        private bool isDisposed;

        internal static TableType ParseType(string type)
        {
            if (type == null)
                return TableType.Default;
            TableType tableType = TableType.Default;
            while (tableType < TableType.Unknown && string.Compare(tableType.ToString(), type, StringComparison.OrdinalIgnoreCase) != 0)
                ++tableType;
            return tableType;
        }

        internal static Table CreateInstance(string name, Database parentDatabase, ClusteredRowset clonedRowset, TableType type)
        {
        ...
        private Status RowStatus
        {
            set
            {
                if (value == Status.SynchPosition || rowStatus == Status.SynchPosition && value == Status.NoAction)
                {
                    rowStatus = value;
                }
                else
                {
                    if (value == Status.InsertBlank)
                    ...
                    if (value == Status.Update)
                    {
                        switch (rowStatus)
                        {
                            case Status.Insert:
                                return;
                            case Status.InsertBlank:
                                rowStatus = Status.Insert;
                                return;
                            default:
                                if (rowStatus != Status.Update)
                                {
                                    Rowset.PrepareEditStatus();
                                    break;
                                }
        ...
        internal void AddOrder(string name, Index order)
        {
            if (order.IsPrimary)
                pkOrder = order;
            Add(name, order);
        ...
        private void RemoveOrder(Index order)
        {
            Remove(order.Alias);
            order.Dispose();
            if (pkOrder != order)
        ...
        private Index GetOrder(string name)
        {
            return GetOrder(name, true);
        }

        private Index GetOrder(string name, bool raiseError)
        {
            if (name == null)
                return Rowset;
            if (ContainsKey(name))
        ...
        private bool LookForKey(Row key, Index index, bool partialMatching, bool softPosition, bool releaseIndexLock)
        {
        ...
        {
            if (rowStatus != Status.SynchPosition)
                return;
        ...
        internal void DeclareIndex(string name, string keyExpression, string forExpression, bool primary, bool unique, bool v_index, bool sparse, bool fk_constraint, bool fts, bool temporary, int lcid)
        {
            ...
            keyExpression = Index.FixKeyExpression(keyExpression);
            if (!temporary)
            {
            ...
            IVistaDBIndexInformation indexInformation = new TableSchema.IndexCollection.IndexInformation("", name, keyExpression, unique || primary, primary, false, sparse, fk_constraint, fts, temporary, 0UL, null);
        ...
        internal void DropIndex(string name, bool commit)
        {
            ...
            RemoveOrder((Index)order);
        }

        private void DropFTSIndex()
        {
            foreach (Index order in Values)
            {
                if (order.IsFts)
                {
        ...
        internal Index GetIndexOrder(string name, bool raiseError)
        {
        ...
        private string SetModifyProcessing(string constraint)
        {
            if (constraint == null)
                return null;
            Rowset.ActivateConstraint(exportConstraint, constraint, 1, Rowset, false);
        ...
        internal void CascadeUpdateForeignTable(Index primaryIndex, string order, VistaDBReferentialIntegrity integrity)
        {
        ...
        internal void CascadeDeleteForeignTable(Index primaryIndex, string order)
        {
            RowsetIndex order1 = (RowsetIndex)GetOrder(order);
            Rowset.LockStorage();
            try
        ...
        {
            ActiveOrder.Top();
            RowStatus = Status.SynchPosition;
        }
        ...
        {
            Index index = indexName == null ? ActiveOrder : GetOrder(indexName);
            if (index == Rowset)
                throw new VistaDBException(272, Rowset.Name);
        ...
        {
            Index index = string.IsNullOrEmpty(indexName) ? ActiveOrder : GetOrder(indexName);
            if (index == Rowset)
                throw new VistaDBException(272, Rowset.Name);
        ...
        IVistaDBIndexCollection IVistaDBTable.TemporaryIndexes
        {
            get
            {
                TableSchema.IndexCollection indexCollection = new TableSchema.IndexCollection();
                foreach (Index index in Values)
                {
        ...
        internal class TableSchema : List<Row.Column>, IVistaDBTableSchema, IVistaDBDatabaseObject, IEnumerable<IVistaDBColumnAttributes>, IEnumerable, IDisposable
        {
            ...
            private IndexCollection indexes = new IndexCollection();
            private IdentityCollection identities = new IdentityCollection();
            private DefaultValueCollection defaults = new DefaultValueCollection();
            ...
            internal TableSchema(string name, TableType type, string description, ulong tableId, DataStorage linkedStorage)
            {
                if (name == null)
            ...
            internal IndexCollection Indexes
            {
                get
                {
                    return indexes;
            ...
            internal IdentityCollection Identities
            {
                get
                {
                    return identities;
            ...
            internal DefaultValueCollection Defaults
            {
                get
                {
                    return defaults;
            ...
            internal ConstraintCollection Constraints
            {
                get
                {
                    return constraints;
            ...
            bool IVistaDBTableSchema.IsSystemTable
            {
                get
                {
                    if (type != TableType.Anchor)
                        return type == TableType.Tombstone;
                    return true;
                }
            }
            ...
            IVistaDBIndexInformation IVistaDBTableSchema.DefineIndex(string name, string keyExpression, bool primary, bool unique)
            {
                ...
                {
                    ((IndexCollection.IndexInformation)indexInformation1).KeyExpression = keyExpression;
                    ((IndexCollection.IndexInformation)indexInformation1).Unique = unique;
                    ((IndexCollection.IndexInformation)indexInformation1).Primary = primary;
                    ((IndexCollection.IndexInformation)indexInformation1).Sparse = false;
                    return indexInformation1;
                }
                if (primary)
                {
                    foreach (IndexCollection.IndexInformation indexInformation2 in indexes.Values)
                    {
                        if (indexInformation2.Primary)
                            throw new VistaDBException(143, indexInformation2.Name);
                    }
                }
                IVistaDBIndexInformation indexInformation3 = new IndexCollection.IndexInformation("", name, keyExpression, unique, primary, false, false, false, false, false, 0UL, null);
            ...
            void IVistaDBTableSchema.DefineIdentity(string columnName, string seedValue, string stepExpression)
            {
                ...
                {
                    ((IdentityCollection.IdentityInformation)identityInformation1).seedValue = seedValue;
                    ((IdentityCollection.IdentityInformation)identityInformation1).stepExpression = stepExpression;
                }
                else
                {
                    IVistaDBIdentityInformation identityInformation2 = new IdentityCollection.IdentityInformation(null, null, columnName, seedValue, stepExpression);
            ...
            void IVistaDBTableSchema.DefineDefaultValue(string columnName, string scriptExpression, bool useInUpdate, string description)
            {
                ...
                {
                    ((DefaultValueCollection.DefaultValueInformation)valueInformation1).Expression = scriptExpression;
                    ((DefaultValueCollection.DefaultValueInformation)valueInformation1).UseInUpdate = useInUpdate;
                    ((Database.DatabaseObject)valueInformation1).Description = description;
                }
                else
                {
                    IVistaDBDefaultValueInformation valueInformation2 = new DefaultValueCollection.DefaultValueInformation(columnName, scriptExpression, useInUpdate, description);
            ...
            void IVistaDBTableSchema.DefineConstraint(string name, string scriptExpression, string description, bool insert, bool update, bool delete)
            {
                ...
                {
                    ((ConstraintCollection.ConstraintInformation)constraintInformation).Expression = scriptExpression;
                    ((Database.DatabaseObject)constraintInformation).Description = description;
                    ((ConstraintCollection.ConstraintInformation)constraintInformation).Option = option;
                }
                else
                {
                    constraintInformation = new ConstraintCollection.ConstraintInformation(name, scriptExpression, description, option);
            ...
            internal class IndexCollection : VistaDBKeyedCollection<string, IVistaDBIndexInformation>, IVistaDBIndexCollection, IVistaDBKeyedCollection<string, IVistaDBIndexInformation>, ICollection<IVistaDBIndexInformation>, IEnumerable<IVistaDBIndexInformation>, IEnumerable
            {
                ...
                internal class IndexInformation : Database.DatabaseObject, IVistaDBIndexInformation, IVistaDBDatabaseObject
                {
                    ...
                    private bool temporary;
                    private bool fts;
                    private KeyColumnInformation[] keyStructure;
                      ...
                      : base(Database.VdbObjects.Index, name, null)
                    {
                        ...
                        if (keyStruct == null)
                            return;
                        keyStructure = new KeyColumnInformation[keyStruct.Length / 2];
                        for (int index1 = 0; index1 < keyStructure.Length; ++index1)
                        {
                            int index2 = 2 * index1;
                            keyStructure[index1] = new KeyColumnInformation(keyStruct[index2], keyStruct[index2 + 1]);
                        }
                    }
                    ...
                    internal static string MakeUpKey(IVistaDBKeyColumn[] keyStruct, IVistaDBTableSchema schema)
                    {
                        ...
                        return Index.FixKeyExpression(keyExpression);
                    }

                    internal void MakeUpKeyExpression(IVistaDBTableSchema schema)
                    {
                        keyExpression = MakeUpKey(keyStructure, schema);
                    }
                }
            }
            ...
            internal class DefaultValueCollection : VistaDBKeyedCollection<string, IVistaDBDefaultValueInformation>, IVistaDBDefaultValueCollection, IVistaDBKeyedCollection<string, IVistaDBDefaultValueInformation>, ICollection<IVistaDBDefaultValueInformation>, IEnumerable<IVistaDBDefaultValueInformation>, IEnumerable
            {
                ...
                internal class DefaultValueInformation : Database.DatabaseObject, IVistaDBDefaultValueInformation, IVistaDBDatabaseObject
                {
                    ...
                    public override bool Equals(object obj)
                    {
                        if (base.Equals(obj) && string.Compare(expression, ((DefaultValueInformation)obj).expression) == 0)
                            return useInUpdate == ((DefaultValueInformation)obj).useInUpdate;
                        return false;
                    }
            ...
            internal class IdentityCollection : VistaDBKeyedCollection<string, IVistaDBIdentityInformation>, IVistaDBIdentityCollection, IVistaDBKeyedCollection<string, IVistaDBIdentityInformation>, ICollection<IVistaDBIdentityInformation>, IEnumerable<IVistaDBIdentityInformation>, IEnumerable
            {
                ...
                internal class IdentityInformation : Database.DatabaseObject, IVistaDBIdentityInformation, IVistaDBDatabaseObject
                {
                    ...
                    private GetSeedValue getSeedValue;

                    internal IdentityInformation(GetSeedValue getSeedValue, string tableName, string columnName, string seedValue, string stepExpression)
                    ...
                    internal bool CopySeedValue(IdentityInformation originalIdentity)
                    {
                    ...
                    public override bool Equals(object obj)
                    {
                        ...
                        if (base.Equals(obj) && string.Compare(seedValue, identityInformation.seedValue) == 0 && string.Compare(stepExpression, identityInformation.stepExpression) == 0)
                            return EqualNames(tableName, identityInformation.tableName);
                        return false;
                    }
            ...
            internal class ConstraintCollection : VistaDBKeyedCollection<string, IVistaDBConstraintInformation>, IVistaDBConstraintCollection, IVistaDBKeyedCollection<string, IVistaDBConstraintInformation>, ICollection<IVistaDBConstraintInformation>, IEnumerable<IVistaDBConstraintInformation>, IEnumerable
            {
                ...
                internal class ConstraintInformation : Database.DatabaseObject, IVistaDBConstraintInformation, IVistaDBDatabaseObject
                {
                    ...
                    public override bool Equals(object obj)
                    {
                        ConstraintInformation constraintInformation = (ConstraintInformation)obj;
        ...
        internal class ContextStack : List<ContextStack.TableContext>
        {
            ...
            internal void Push()
            {
                Add(new TableContext(parentTable.activeOrder, parentTable.rowStatus));
            }
            ...
            internal class TableContext
            {
                ...
                internal TableContext(Index activeOrder, Status status)
                {
                    Init(activeOrder, status);
                }

                internal Index Restore(ref Status status)
                {
                ...
                internal void Init(Index activeOrder, Status status)
                {
                    this.status = status;
