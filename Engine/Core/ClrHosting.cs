...
namespace VistaDB.Engine.Core
{
    internal class ClrHosting : InsensitiveHashtable
    {
        ...
        internal ClrProcedure this[string clrName]
        {
            get
            {
                return Contains(clrName) ? (ClrProcedure)this[(object)clrName] : (ClrProcedure)(object)null;
            }
        }
                ...
                if (type.IsPublic)
                {
                    foreach (MethodInfo method in type.GetMethods())
                    {
                        if (method.IsStatic && type.IsPublic && method.MemberType == MemberTypes.Method)
                        {
                            ...
                            procedureCollection.AddProcedure("Not assigned #" + num.ToString(), Database.ClrProcedureCollection.ClrProcedureInformation.CompileSignature(fullCLRProcedureName, method.ToString()), null, Encoding.Unicode.GetBytes(assemblyName), false);
                            ++num;
                        }
                    }
                }
            ...
            foreach (Type type in types)
            {
                if (type.IsPublic)
                {
                    foreach (MethodInfo method in type.GetMethods())
                    {
                        ...
                        {
                            string fullCLRProcedureName = type.FullName + "." + method.Name;
                            triggerCollection.AddTrigger("Not assigned #" + num.ToString(), Database.ClrProcedureCollection.ClrProcedureInformation.CompileSignature(fullCLRProcedureName, method.ToString()), null, Encoding.Unicode.GetBytes(assemblyName), false, Row.EmptyReference, 0L);
                            ++num;
                        }
                    }
                }
        ...
        private void EFInitialize()
        {
            ...
            SystemFunctions.RegisterIntoHosting(this, _sysProcs);
        }
        ...
        internal IVistaDBClrProcedureCollection EnumSystemProcs()
        {
            ...
            foreach (string sysProc in _sysProcs)
            {
                ...
                procedureCollection.AddProcedure(sysProc, clrProcedure.Method.Name + char.MinValue, null, Encoding.Unicode.GetBytes(clrProcedure.ParentAssembly.FullName), true);
            }
            return procedureCollection;
        }
        ...
        internal class ClrProcedure
        {
            ...
            internal object Execute(params object[] prms)
            {
                try
                {
                    ...
                    return procedure.Invoke(null, prms);
                }
                catch
                {
                    throw;
            ...
            {
                return procedure.Invoke(null, null);
            }
            ...
            internal object ExecFillRow(params object[] prms)
            {
                try
                {
                    ...
                    return fillRow.Invoke(null, prms);
                }
                catch (Exception)
                {
                    throw;
