...
namespace VistaDB.Engine.Core
{
  internal static class VistaDBLicensing
  {
    ...
    private const string FeatureKeyFilesMatchFormat = "Feature_{0}_{1}_???.glk";
    private const string FeatureKeyFileNameFormat = "Feature_{0}_{1}_{2:000}.glk";
    ...
    private static volatile bool g_TriedFixActivationKey;
    private static volatile LicenseActivationStatus g_LicensingStatus;

    static VistaDBLicensing()
    {
      VistaDBLicensing.g_LicensingBasePath = Path.Combine(VistaDBLicensing.g_LicensingBasePath, "Gibraltar");
      VistaDBLicensing.g_LicensingBasePath = Path.Combine(VistaDBLicensing.g_LicensingBasePath, "Licensing");
    }
    ...
    internal static bool HasLicensingFolder
    {
      get
      {
        try
        {
          return Directory.Exists(VistaDBLicensing.g_LicensingBasePath);
        }
        catch
        {
          return false;
    ...
    {
      if (VistaDBLicensing.g_LicensingStatus >= LicenseActivationStatus.MinimumAcceptedLicense || !Debugger.IsAttached)
        return;
      if (VistaDBLicensing.g_LicensingNextCheck <= DateTimeOffset.Now)
      {
        if (VistaDBLicensing.CheckLicenseActivation(1, 1))
          VistaDBLicensing.g_LicensingStatus = LicenseActivationStatus.MinimumAcceptedLicense;
        VistaDBLicensing.g_LicensingNextCheck = DateTimeOffset.Now.AddSeconds(10.0);
      }
      if (VistaDBLicensing.g_LicensingStatus < LicenseActivationStatus.MinimumAcceptedLicense)
    ...
    {
      if (!VistaDBLicensing.HasLicensingFolder)
        return false;
      string path2 = string.Format("Feature_{0}_{1}_{2:000}.glk", (object) VistaDBLicensing.VistaDBProductCode, (object) applicationCode, (object) featureCode);
      string path = Path.Combine(VistaDBLicensing.g_LicensingBasePath, path2);
      if (!File.Exists(path))
        return false;
      ...
      return applicationFeatureKey.HasValidChecksum && applicationFeatureKey.ProductCode == VistaDBLicensing.VistaDBProductCode && (applicationFeatureKey.ApplicationCode == applicationCode && applicationFeatureKey.FeatureCode == featureCode) && (!activationKey.ExpirationDt.HasValue || !(activationKey.ExpirationDt.Value < DateTimeOffset.Now));
    }
    ...
    {
      VistaDBLicensing.FixKeyPermissions(containerInfo.UniqueKeyContainerName, poison, (string) null, persisting);
    }

    [StrongNameIdentityPermission(SecurityAction.LinkDemand, PublicKey = "")]
    internal static void FixKeyPermissions(string uniqueKeyName, bool poison, string label, bool persisting)
    {
      VistaDBLicensing.ValidateCallStack(2);
      ...
      try
      {
        ...
        if (files.Length != 1)
          return;
        SecurityIdentifier securityIdentifier1 = new SecurityIdentifier(WellKnownSidType.BuiltinUsersSid, (SecurityIdentifier) null);
        SecurityIdentifier securityIdentifier2 = new SecurityIdentifier(WellKnownSidType.BuiltinAdministratorsSid, (SecurityIdentifier) null);
        SecurityIdentifier securityIdentifier3 = new SecurityIdentifier(WellKnownSidType.CreatorOwnerSid, (SecurityIdentifier) null);
        FileSystemAccessRule rule1 = new FileSystemAccessRule((IdentityReference) securityIdentifier1, FileSystemRights.Modify, AccessControlType.Allow);
        FileSystemAccessRule rule2 = new FileSystemAccessRule((IdentityReference) securityIdentifier2, FileSystemRights.Modify, AccessControlType.Allow);
        FileSystemAccessRule rule3 = new FileSystemAccessRule((IdentityReference) securityIdentifier3, FileSystemRights.FullControl, AccessControlType.Allow);
          ...
          try
          {
            ...
            else if (File.Exists(path2))
            {
              ...
              foreach (AuthorizationRule accessRule in (ReadOnlyCollectionBase) accessControl.GetAccessRules(true, false, typeof (SecurityIdentifier)))
              {
                ...
                {
                  if (systemAccessRule.IdentityReference == (IdentityReference) securityIdentifier1)
                    flag1 = false;
                  else if (systemAccessRule.IdentityReference == (IdentityReference) securityIdentifier2)
                    flag2 = false;
                }
              }
              if (!flag1)
      ...
      try
      {
        ...
        for (int skipFrames = 1; skipFrames <= frameCount; ++skipFrames)
        {
          ...
          if (base64String2 != "38k1r+ISVGE=" && base64String2 != "ykKh7o0uQtM=")
            throw new VistaDBLicenseException("Invalid call stack: Public key did not match: " + VistaDBLicensing.ToBase16Transform(name.GetPublicKey()));
          if (skipFrames >= frameCount)
          {
            foreach (StrongNameIdentityPermissionAttribute permissionAttribute in Attribute.GetCustomAttributes((MemberInfo) method, typeof (StrongNameIdentityPermissionAttribute)) ?? new Attribute[0])
            {
