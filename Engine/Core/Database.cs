...
namespace VistaDB.Engine.Core
{
    internal class Database : ClusteredRowset
    {
        ...
        private IdentityCollection lastIdentities = new IdentityCollection();
        private IdentityCollection lastTimeStamps = new IdentityCollection();
        private TableIdMap transferList = new TableIdMap(10);
        ...
        private EnumCache<IViewList> _viewsCache = new EnumCache<IViewList>();
        private EnumCache<IStoredProcedureCollection> _spCache = new EnumCache<IStoredProcedureCollection>();
        private EnumCache<IUserDefinedFunctionCollection> _udfCache = new EnumCache<IUserDefinedFunctionCollection>();
        ...
        {
            ++UniqueId;
            Database database = new Database(fileName, "DB" + UniqueId.ToString(), parentConnection, new Parser(parentConnection), Encryption.CreateEncryption(cryptoKey), toPack);
        ...
        new internal DatabaseHeader Header
        {
            get
            {
                return (DatabaseHeader)base.Header;
            }
        }
        ...
        {
            IVistaDBTableSchema vistaDbTableSchema = new Table.TableSchema(SystemSchema, ActivateSyncMode ? Table.TableType.Tombstone : Table.TableType.Default, null, 0UL, this);
        ...
        private void ActivateRelationships(Table table, IVistaDBTableSchema schema)
        {
            ...
            foreach (IVistaDBRelationshipInformation relationship in schema.ForeignKeys.Values)
            {
                try
                {
                    if (DatabaseObject.EqualNames(relationship.PrimaryTable, rowset.Name))
                    ...
                    if (DatabaseObject.EqualNames(relationship.ForeignTable, rowset.Name))
                    {
                        rowset.ActivateForeignKey(table, relationship.Name, relationship.PrimaryTable);
                        if (DatabaseObject.EqualNames(relationship.PrimaryTable, relationship.ForeignTable))
        ...
        private void CreateObjectEntry(bool commit, VdbObjects type, ulong foreignReference, string name, ulong objectId, ulong reference, long options, bool status, string scriptValue, string description, byte[] binary, byte[] blob)
        {
            PrepareEditStatus();
        ...
        private void UpdateObjectEntry(bool commit, VdbObjects type, ulong foreignReference, string name, ulong objectId, ulong reference, long options, bool status, string scriptValue, string description, byte[] binary, byte[] blob)
        {
            FillRowData(SatelliteRow, type, foreignReference, name, objectId, reference, options, status, scriptValue, description, binary, blob);
        ...
        private void UpdateFullObjectEntry(bool commit, VdbObjects oldType, ulong oldForeignReference, string oldName, ulong oldObjectId, ulong oldReference, long oldOptions, bool oldStatus, string oldScriptValue, string oldDescription, byte[] oldBinary, byte[] oldBlob, VdbObjects newType, ulong newForeignReference, string newName, ulong newObjectId, ulong newReference, long newOptions, bool newStatus, string newScriptValue, string newDescription, byte[] newBinary, byte[] newBlob)
        {
        ...
        private void DeleteInScope(VdbObjects type, ulong referencingObject, string name, bool commit)
        {
            if (name != null)
        ...
        {
            FillRowData(CurrentRow, VdbObjects.Table, Row.EmptyReference, name, 0UL, Row.EmptyReference, 0L, true, null, null, null, null);
            ...
            try
            {
                ...
                if (column.IsNull || (int)column.Value != 1 || !DatabaseObject.EqualNames(name, (string)CurrentRow[nameIndex].Value))
        ...
        private void FillRowData(Row key, VdbObjects type, ulong foreignReference, string name, ulong objectId, ulong reference, long options, bool status, string scriptValue, string description, byte[] binary, byte[] blob)
        {
            if (name != null)
                name = Row.Column.FixName(name);
        ...
        private void TraverseDiskObjects(VdbObjects type, ulong referencingObject, TraverseJobDelegate job, params object[] hints)
        {
            try
            {
                TopRow.InitTop();
                ...
                TopRow[foreignReferenceIndex].Value = (long)referencingObject;
                BottomRow[typeIndex].Value = type;
                BottomRow[foreignReferenceIndex].Value = type == VdbObjects.Relationship || type == VdbObjects.CLRTrigger ? (long)hints[1] : (long)referencingObject;
                MoveToRow(TopRow);
        ...
        private bool LoadViewsJob(Row currentRow, params object[] parms)
        {
            ...
            ((ViewList)parms[0]).Add(viewName, script, scriptExtension, description);
            return true;
        }
        ...
        private bool LoadSqlStoredProceduresJob(Row currentRow, params object[] parms)
        {
            ...
            ((StoredProcedureCollection)parms[0]).AddProcedure(name, scriptData, description, status);
            return true;
        }
        ...
        private bool LoadSqlFunctionsJob(Row currentRow, params object[] parms)
        {
            ...
            ((UdfCollection)parms[0]).AddFunction(name, scriptData, num == 0UL, description, status);
            return true;
        }
        ...
        private bool LoadClrProceduresJob(Row currentRow, params object[] parms)
        {
            ...
            bool status = (bool)currentRow[statusIndex].Value;
            ClrProcedureCollection parm1 = (ClrProcedureCollection)parms[0];
        ...
        private bool LoadClrTriggersJob(Row currentRow, params object[] parms)
        {
            ...
            byte[] assemblyFullName = (byte[])currentRow[extraBinaryIndex].Value;
            bool status = (bool)currentRow[statusIndex].Value;
            ClrTriggerCollection parm1 = (ClrTriggerCollection)parms[0];
            int parm2 = (int)parms[2];
        ...
        private bool LoadAssembliesJob(Row currentRow, params object[] parms)
        {
            ...
            string script = (string)currentRow[scriptValueIndex].Value;
            string description = (string)currentRow[descriptionIndex].Value;
            ((AssemblyCollection)parms[0]).AddAssembly(assemblyName, assemblyBody, script, description, ParentConnection);
            return true;
        }
        ...
        private bool LoadTablesJob(Row currentRow, params object[] parms)
        {
            ...
            ((TableIdMap)parms[0]).Add(key, num);
            return true;
        }
        ...
        private bool LoadRelationshipsJob(Row currentRow, params object[] parms)
        {
            ...
            string description = (string)currentRow[descriptionIndex].Value;
            IVistaDBRelationshipInformation relationshipInformation = new RelationshipCollection.RelationshipInformation(str, foreignTableId, foreignKey, primaryTableId, options, description);
        ...
        private bool LoadIndexesJob(Row currentRow, params object[] parms)
        {
            ...
            bool unique = IndexHeader.IsUnique(signature);
            bool primary = IndexHeader.IsPrimary(signature);
            bool descend = false;
            bool sparse = IndexHeader.IsSparse(signature);
            bool fk_constraint = IndexHeader.IsForeignKey(signature);
            bool fts = IndexHeader.IsFts(signature);
            bool temporary = false;
        ...
        private void GetIdentities(ulong tableId, IVistaDBIdentityCollection identities, string tableName)
        {
            ...
            TraverseDiskObjects(VdbObjects.Identity, tableId, new TraverseJobDelegate(LoadIdentitiesJob), identities, tableName, new Table.TableSchema.IdentityCollection.IdentityInformation.GetSeedValue(GetSeedValue));
        }
        ...
        {
            triggers.Clear();
            TraverseDiskObjects(VdbObjects.CLRTrigger, tableId, new TraverseJobDelegate(LoadClrTriggersJob), triggers, (long)tableId, VdbObjects.CLRTrigger);
            foreach (ClrTriggerCollection.ClrTriggerInformation triggerInformation in triggers.Values)
                triggerInformation.ParentTable = tableName;
        }

        private void GetRelationships(ulong foreignTableId, RelationshipCollection relationships)
        {
            relationships.Clear();
            TraverseDiskObjects(VdbObjects.Relationship, foreignTableId, new TraverseJobDelegate(LoadRelationshipsJob), relationships, foreignTableId == 0UL ? long.MaxValue : (long)foreignTableId);
            CompleteRelationshipInfo(relationships);
        }

        private void CompleteRelationshipInfo(RelationshipCollection relationships)
        {
            if (relationships.Count == 0)
                return;
            TableIdMap tableIdMap = GetTableIdMap();
            foreach (RelationshipCollection.RelationshipInformation relationshipInformation in relationships.Values)
            {
        ...
        private void FillOutDatabaseSchema(DataSet database)
        {
            ...
            foreach (IVistaDBRelationshipInformation relationshipInformation in GetRelationships().Values)
            {
                if (transferList.Contains(relationshipInformation.PrimaryTable) && transferList.Contains(relationshipInformation.ForeignTable))
                {
                    ...
                    childKeyConstraint.UpdateRule = (Rule)relationshipInformation.UpdateIntegrity;
                    childKeyConstraint.DeleteRule = (Rule)relationshipInformation.DeleteIntegrity;
        ...
        private void FillInDatabaseSchema(DataSet database)
        {
            ...
            foreach (DataTable table in database.Tables)
            {
                ...
                foreach (string key in extendedProperties1.Keys)
                {
                    ...
                    if (!DatabaseObject.EqualNames(dataSet.DataSetName, "description") && !DatabaseObject.EqualNames(dataSet.DataSetName, "tableType"))
                    {
                        ...
                        if (DatabaseObject.EqualNames(dataSet.DataSetName, "indexes"))
                        {
                        ...
                        if (DatabaseObject.EqualNames(dataSet.DataSetName, "identities"))
                        {
                            foreach (DataColumn column in dataSet.Tables[0].Columns)
                            {
                                PropertyCollection extendedProperties2 = column.ExtendedProperties;
                        ...
                        else if (DatabaseObject.EqualNames(dataSet.DataSetName, "constraints"))
                        {
                            foreach (DataColumn column in dataSet.Tables[0].Columns)
                            {
                                PropertyCollection extendedProperties2 = column.ExtendedProperties;
                        ...
                        else if (DatabaseObject.EqualNames(dataSet.DataSetName, "defaultValues"))
                        {
                            foreach (DataColumn column in dataSet.Tables[0].Columns)
                            {
                                PropertyCollection extendedProperties2 = column.ExtendedProperties;
                ...
                {
                    if (!(constraint is System.Data.ForeignKeyConstraint) && constraint is UniqueConstraint && DatabaseObject.EqualNames(table.TableName, constraint.Table.TableName))
                    {
            ...
            if (database.Relations.Count > 0)
            {
                ...
                foreach (DataRelation relation in database.Relations)
                {
                    ...
                    relationships.Add(relation.RelationName, new RelationshipCollection.RelationshipInformation(relation.RelationName, tableSchema2.HeaderPosition, foreignKey, tableSchema1.HeaderPosition, RelationshipCollection.RelationshipInformation.MakeOptions((VistaDBReferentialIntegrity)childKeyConstraint.UpdateRule, (VistaDBReferentialIntegrity)childKeyConstraint.DeleteRule), (string)childKeyConstraint.ExtendedProperties["description"])
                    {
            ...
            {
                if (DatabaseObject.EqualNames(key, "databaseObjects"))
                {
                    DataSet dataSet = new DataSet(key);
                    ...
                    {
                        if (DatabaseObject.EqualNames(table.TableName, "views"))
                        {
                            foreach (DataRow row in (InternalDataCollectionBase)table.Rows)
                            {
                                string name = (string)row[0];
                                string str = (string)row[1];
                                IView view = new ViewList.View(name);
                                view.Expression = str;
                                RegisterViewObject(view);
                            }
                        }
                        if (DatabaseObject.EqualNames(table.TableName, "assemblies"))
                        {
                            foreach (DataRow row in (InternalDataCollectionBase)table.Rows)
                            {
                        ...
                        if (DatabaseObject.EqualNames(table.TableName, "clrProcs"))
                        {
                            foreach (DataRow row in (InternalDataCollectionBase)table.Rows)
                            {
        ...
        {
            if (DatabaseObject.EqualNames(oldSchema.Name, newSchema.Name, false) && string.Compare(oldSchema.Description, newSchema.Description, StringComparison.OrdinalIgnoreCase) == 0)
                return false;
            ...
            UpdateFullObjectEntry(commit, VdbObjects.Table, Row.EmptyReference, oldSchema.Name, headerPosition, Row.EmptyReference, type1, true, null, oldSchema.Description, null, null, VdbObjects.Table, Row.EmptyReference, newSchema.Name, headerPosition, Row.EmptyReference, type2, true, null, newSchema.Description, null, null);
            return true;
        }
        ...
        private bool UpdateIndexesAndRelations(Table srcTable, AlterList alterList, bool commit)
        {
            ...
            foreach (string key in alterList.UpdatedIndexes.Keys)
            {
                ...
                {
                    RelationshipCollection.RelationshipInformation foreignKey = (RelationshipCollection.RelationshipInformation)alterList.OldSchema.ForeignKeys[key];
                    RelationshipCollection.RelationshipInformation newRelation = new RelationshipCollection.RelationshipInformation(foreignKey.Name, foreignKey.ForeignTableId, updatedIndex.KeyExpression, foreignKey.PrimaryTableId, foreignKey.Options, foreignKey.Description);
            ...
            foreach (Table.TableSchema.IndexCollection.IndexInformation indexInformation1 in indexes1.Values)
            {
                ...
                if (indexes2.TryGetValue(name1, out IVistaDBIndexInformation indexInformation2) && indexInformation2 != null && alterList.PersistentIndexes.ContainsKey(name1))
                {
                    ...
                    else
                    {
                        ...
                        string str1 = FixKeyExpression(keyExpression);
                        if (str1.Length == 0)
                        {
                            alterList.PersistentIndexes.Remove(name1);
                ...
                if (constraints2.TryGetValue(name1, out IVistaDBConstraintInformation constraintInformation2) && alterList.PersistentConstraints.ContainsKey(name1))
                {
                    ...
                    foreach (IVistaDBColumnAttributes columnAttributes in alterList.OldSchema)
                    {
                        ...
                        if (alterList.TryGetValue(name2, out AlterList.AlterInformation alterInformation) && alterInformation.NewColumn != null)
                        {
                            ...
                            if (!DatabaseObject.EqualNames(name2, name3, true))
                            {
        ...
        private RelationshipCollection UpdateRelationships(AlterList alterList, Table.TableSchema modifiedSchema, bool commit)
        {
            string name = alterList.OldSchema.Name;
            Dictionary<string, RelationshipCollection.RelationshipInformation> dictionary1 = new Dictionary<string, RelationshipCollection.RelationshipInformation>();
            Dictionary<string, RelationshipCollection.RelationshipInformation> dictionary2 = new Dictionary<string, RelationshipCollection.RelationshipInformation>();
            RelationshipCollection relationships = (RelationshipCollection)GetRelationships();
            RelationshipCollection foreignKeys = alterList.NewSchema.ForeignKeys;
            foreach (RelationshipCollection.RelationshipInformation relationshipInformation1 in relationships.Values)
            {
                ...
                else if (DatabaseObject.EqualNames(name, relationshipInformation1.PrimaryTable))
                {
                    RelationshipCollection.RelationshipInformation relationshipInformation2 = new RelationshipCollection.RelationshipInformation(relationshipInformation1.Name, relationshipInformation1.ForeignTableId, relationshipInformation1.ForeignKey, modifiedSchema.HeaderPosition, relationshipInformation1.Options, relationshipInformation1.Description);
                ...
                else if (DatabaseObject.EqualNames(name, relationshipInformation1.ForeignTable))
                {
                    ...
                    {
                        relationshipInformation2 = new RelationshipCollection.RelationshipInformation(relationshipInformation1.Name, headerPosition, keyExpression, relationshipInformation1.PrimaryTableId, relationshipInformation1.Options, relationshipInformation1.Description);
                    }
                    else
                    {
            ...
            foreach (RelationshipCollection.RelationshipInformation relationshipInformation in dictionary1.Values)
                DeleteInScope(VdbObjects.Relationship, relationshipInformation.ForeignTableId, relationshipInformation.Name, false);
            foreignKeys.Clear();
            foreach (RelationshipCollection.RelationshipInformation newRelation in dictionary2.Values)
            {
                RelationshipCollection.RelationshipInformation oldRelation = (RelationshipCollection.RelationshipInformation)relationships[newRelation.Name];
                ulong headerPosition = modifiedSchema.HeaderPosition;
                ...
                {
                    DeleteInScope(VdbObjects.Relationship, oldRelation.ForeignTableId, oldRelation.Name, false);
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (indexInformation2 == null)
                        return row;
                    RowsetIndex rowsetIndex = (RowsetIndex)((Dictionary<string, Index>)table)[indexInformation2.Name];
                    ...
                    foreach (IVistaDBKeyColumn vistaDbKeyColumn1 in keyStructure1)
                    {
                        ...
                        {
                            if (DatabaseObject.EqualNames(((Table)table).Rowset.DefaultRow[vistaDbKeyColumn2.RowIndex].Name, name2))
                            {
                                flag2 = true;
                                break;
                            }
        ...
        {
            DeleteInScope(VdbObjects.Identity, headerPosition, null, commit);
            DeleteInScope(VdbObjects.DefaultValue, headerPosition, null, commit);
            DeleteInScope(VdbObjects.Constraint, headerPosition, null, commit);
            DeleteInScope(VdbObjects.Relationship, headerPosition, null, commit);
            DeleteInScope(VdbObjects.Index, headerPosition, null, commit);
            DeleteInScope(VdbObjects.Column, headerPosition, null, commit);
            DeleteInScope(VdbObjects.CLRTrigger, headerPosition, null, commit);
            DeleteInScope(VdbObjects.Table, Row.EmptyReference, name, commit);
        ...
        private void UnregisterTable(string name, bool commit, bool raiseNonExistence, bool testPKRefernces, bool raiseOtherErrors)
        {
            ...
            try
            {
                ...
                if (raiseOtherErrors)
                {
                    ...
                    string name1 = SyncExtension.TombstoneTablenamePrefix + name;
                    ulong tableId2 = GetTableId(SyncExtension.TombstoneTablenamePrefix + name, out description, out type, false);
        ...
        private void UpdateColumn(IVistaDBColumnAttributes oldColumn, IVistaDBColumnAttributes newColumn, ulong storageId, bool commit)
        {
            ...
            UpdateFullObjectEntry(commit, VdbObjects.Column, storageId, oldColumn.Name, (ulong)oldColumn.RowIndex, Row.EmptyReference, (long)num, true, oldColumn.Caption, oldColumn.Description, null, null, VdbObjects.Column, storageId, newColumn.Name, (ulong)newColumn.RowIndex, Row.EmptyReference, (long)num, true, newColumn.Caption, newColumn.Description, null, null);
        }
        ...
        private void UpdateRelationship(RelationshipCollection.RelationshipInformation oldRelation, RelationshipCollection.RelationshipInformation newRelation, bool commit)
        {
            UpdateFullObjectEntry(commit, VdbObjects.Relationship, oldRelation.ForeignTableId, oldRelation.Name, 0UL, oldRelation.PrimaryTableId, oldRelation.Options, true, ((IVistaDBRelationshipInformation)oldRelation).ForeignKey, oldRelation.Description, null, null, VdbObjects.Relationship, newRelation.ForeignTableId, newRelation.Name, 0UL, newRelation.PrimaryTableId, newRelation.Options, true, ((IVistaDBRelationshipInformation)newRelation).ForeignKey, newRelation.Description, null, null);
        }
        ...
        private void ImportCLRProc(string procedureName, string clrHostedProcedure, string assemblyName, string description, string signature)
        {
            ...
            try
            {
                CreateObjectEntry(false, VdbObjects.CLRProcedure, Row.EmptyReference, procedureName, Row.EmptyReference, Row.EmptyReference, 0L, true, ClrProcedureCollection.ClrProcedureInformation.CompileSignature(clrHostedProcedure, signature), description, Encoding.Unicode.GetBytes(assemblyName), null);
                commit = true;
            }
            catch (Exception ex)
            {
        ...
        private ClrHosting.ClrProcedure PrepareInvoke(string procedureName, ClrHosting list, VdbObjects clrType)
        {
            ...
            switch (clrType)
            {
                case VdbObjects.CLRProcedure:
                    ClrProcedureCollection.ClrProcedureInformation clrProcedureObject = (ClrProcedureCollection.ClrProcedureInformation)LoadClrProcedureObjects()[procedureName];
                    ...
                    break;
                case VdbObjects.CLRTrigger:
                    ClrTriggerCollection.ClrTriggerInformation clrTriggerObject = (ClrTriggerCollection.ClrTriggerInformation)LoadClrTriggerObjects()[procedureName];
            ...
            if (assemblyName == null || clrHostedProcedure == null)
                throw new VistaDBException(clrType == VdbObjects.CLRTrigger ? 397 : 383, procedureName);
            AssemblyCollection.AssemblyInformation assemblyInformation = LocateAssembly(assemblyName, true);
        ...
        private void ImportAssembly(string assemblyName, byte[] coffImage, string fullName, string runtimeVersion, string vistadbVersion, string description)
        {
            ...
            try
            {
                CreateObjectEntry(false, VdbObjects.Assembly, Row.EmptyReference, assemblyName, Row.EmptyReference, Row.EmptyReference, 0L, true, AssemblyCollection.AssemblyInformation.CompileScriptValue(fullName, runtimeVersion, vistadbVersion), description, null, coffImage);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        private AssemblyCollection.AssemblyInformation LocateAssembly(string assemblyName, bool createInstance)
        {
            return (AssemblyCollection.AssemblyInformation)LoadAssemblies(createInstance)[assemblyName];
        }
        ...
        private void ImportCLRTrigger(string triggerName, string clrHostedTrigger, string assemblyName, string tableName, TriggerAction eventType, string description, string signature)
        {
            ...
            try
            {
                CreateObjectEntry(false, VdbObjects.CLRTrigger, GetTableId(tableName, out string description1, out Table.TableType type, true), triggerName, Row.EmptyReference, Row.EmptyReference, (long)eventType, true, ClrProcedureCollection.ClrProcedureInformation.CompileSignature(clrHostedTrigger, signature), description, Encoding.Unicode.GetBytes(assemblyName), null);
                commit = true;
            }
            catch (Exception ex)
            {
        ...
        protected override StorageHeader DoCreateHeaderInstance(int pageSize, CultureInfo culture, DataStorage clonedStorage)
        {
            ...
            return DatabaseHeader.CreateInstance(this, pageSize, culture);
        }
        ...
        internal void CreateRelationships(RelationshipCollection relationships)
        {
            CreateRelationships(relationships, new List<string>());
        }

        internal void CreateRelationships(RelationshipCollection relationships, IList<string> globalNames)
        {
            if (relationships.Count == 0)
                return;
        ...
        {
            string name1 = string.Format(TemporaryName, oldName);
        ...
        internal void RegisterDescription(string description)
        {
            ...
            try
            {
                DeleteInScope(VdbObjects.Description, Row.EmptyReference, null, false);
                if (description == null || description.Length == 0)
                    return;
                CreateObjectEntry(true, VdbObjects.Description, Row.EmptyReference, DescriptionName, Row.EmptyReference, Row.EmptyReference, 0L, true, null, description, null, null);
            }
            finally
            {
                UnlockStorage(true);
        ...
        internal void RegisterRowsetSchema(DataStorage rowset, IVistaDBTableSchema schema)
        {
            ...
            CreateObjectEntry(false, VdbObjects.Table, Row.EmptyReference, rowset.Name, rowset.StorageId, Row.EmptyReference, type, true, null, schema.Description, null, null);
            int num1 = 0;
            foreach (IVistaDBColumnAttributes column in schema)
            {
                ++num1;
                ulong num2 = PrepareColumnOption(column);
                CreateObjectEntry(false, VdbObjects.Column, rowset.StorageId, column.Name, (ulong)column.RowIndex, Row.EmptyReference, (long)num2, true, null, column.Description, null, null);
            }
        }
        ...
        internal bool IsReferencedPK(string pkRowsetName, ref string relation)
        {
            ...
            foreach (RelationshipCollection.RelationshipInformation relationshipInformation in relationships.Values)
            {
                if (DatabaseObject.EqualNames(relationshipInformation.PrimaryTable, pkRowsetName))
                {
        ...
        internal bool IsOuterReferencedPK(string pkRowsetName)
        {
            ...
            foreach (IVistaDBRelationshipInformation relationshipInformation in relationships.Values)
            {
                if (DatabaseObject.EqualNames(relationshipInformation.PrimaryTable, pkRowsetName) && !DatabaseObject.EqualNames(relationshipInformation.ForeignTable, pkRowsetName))
                    return true;
            }
            return false;
        }
        ...
        internal void UpdateRegisteredIndex(RowsetIndex index, IVistaDBIndexInformation newIndex, bool commit)
        {
            ...
            UpdateFullObjectEntry(commit, VdbObjects.Index, index.ParentRowset.StorageId, index.Alias, index.StorageId, Row.EmptyReference, index.Header.Signature, index.Header.Descend, index.KeyExpression, null, index.RowKeyStructure, null, VdbObjects.Index, index.ParentRowset.StorageId, newIndex.Name, index.StorageId, Row.EmptyReference, index.Header.Signature, index.Header.Descend, newIndex.KeyExpression, null, index.RowKeyStructure, null);
        ...
        internal void RegisterIndex(RowsetIndex index)
        {
            ...
            CreateObjectEntry(false, VdbObjects.Index, index.ParentRowset.StorageId, index.Alias, index.StorageId, Row.EmptyReference, index.Header.Signature, index.Header.Descend, scriptValue, null, index.RowKeyStructure, null);
            if (!index.IsPrimary)
                return;
            UpdatePkColumnsFlags(index.ParentRowset.StorageId, index.KeyPCode.EnumColumns(), false);
        }
        ...
        internal void UnregisterIndex(RowsetIndex index)
        {
            ...
            index.ParentRowset.UpdateSchemaVersion();
            DeleteInScope(VdbObjects.Index, index.ParentRowset.StorageId, index.Alias, false);
        }
        ...
        internal void RegisterIdentity(ClusteredRowset rowset, string columnName, string stepExpression)
        {
            ...
            rowset.UpdateSchemaVersion();
            CreateObjectEntry(false, VdbObjects.Identity, rowset.StorageId, columnName, 0UL, Row.EmptyReference, 0L, true, stepExpression, null, null, null);
        }
        ...
        internal void UnregisterIdentity(ClusteredRowset rowset, string columnName, bool checkRegistered)
        {
            ...
            rowset.UpdateSchemaVersion();
            DeleteInScope(VdbObjects.Identity, rowset.StorageId, columnName, false);
        }
        ...
        internal void RegisterDefaultValue(ClusteredRowset rowset, string columnName, string scriptExpression, bool useInUpdate, string description)
        {
            if (IsDefaultValueRegistered(rowset, columnName))
                UnregisterDefaultValue(rowset, columnName, false);
            rowset.UpdateSchemaVersion();
            CreateObjectEntry(false, VdbObjects.DefaultValue, rowset.StorageId, columnName, 0UL, Row.EmptyReference, useInUpdate ? 1L : 0L, true, scriptExpression, description, null, null);
        }
        ...
        internal void UnregisterDefaultValue(ClusteredRowset rowset, string columnName, bool checkRegistered)
        {
            ...
            rowset.UpdateSchemaVersion();
            DeleteInScope(VdbObjects.DefaultValue, rowset.StorageId, columnName, false);
        }
        ...
        internal void RegisterConstraint(ClusteredRowset rowset, string name, string expression, int options, string description)
        {
            ...
            rowset.UpdateSchemaVersion();
            CreateObjectEntry(false, VdbObjects.Constraint, rowset.StorageId, name, 0UL, Row.EmptyReference, options, false, expression, description, null, null);
        }
        ...
        internal void UnregisterConstraint(ClusteredRowset rowset, string name, bool checkRegistered)
        {
            ...
            rowset.UpdateSchemaVersion();
            DeleteInScope(VdbObjects.Constraint, rowset.StorageId, name, false);
        }
        ...
        internal void RegisterForeignKey(string constraintName, ClusteredRowset fkRowset, ClusteredRowset pkRowset, string foreignKey, VistaDBReferentialIntegrity updateIntegrity, VistaDBReferentialIntegrity deleteIntegrity, string description)
        {
            ...
            int num = RelationshipCollection.RelationshipInformation.MakeOptions(updateIntegrity, deleteIntegrity);
            CreateObjectEntry(false, VdbObjects.Relationship, fkRowset.StorageId, constraintName, 0UL, pkRowset.StorageId, num, true, foreignKey, description, null, null);
        }
        ...
        internal void UnregisterForeignKey(string constraintName, ClusteredRowset fkRowset)
        {
            ...
            fkRowset.UpdateSchemaVersion();
            DeleteInScope(VdbObjects.Relationship, fkRowset.StorageId, constraintName, false);
        }

        internal Row AllocateRowsetSchema(ulong storageId, Row rowInstance)
        {
            bool flag = !CaseSensitive;
            TraverseDiskObjects(VdbObjects.Column, storageId, new TraverseJobDelegate(LoadColumnsJob), rowInstance, flag);
        ...
        internal void ModifyForeignTable(bool update, Index primaryIndex, string tableName, string indexName, VistaDBReferentialIntegrity integrity)
        {
        ...
        internal void RegisterViewObject(IView view)
        {
            ...
            try
            {
                ...
                DeleteInScope(VdbObjects.View, Row.EmptyReference, view.Name, false);
                CreateObjectEntry(true, VdbObjects.View, Row.EmptyReference, view.Name, Row.EmptyReference, Row.EmptyReference, 0L, true, scriptValue, view.Description, null, s == null ? null : Encoding.Unicode.GetBytes(s));
            }
            finally
            {
                UnlockStorage(true);
        ...
        internal void UnregisterViewObject(IView view)
        {
            ...
            try
            {
                DeleteInScope(VdbObjects.View, Row.EmptyReference, view.Name, true);
            }
            finally
            {
                UnlockStorage(true);
        ...
        internal void RegisterCLRProc(string procedureName, string clrHostedProcedure, string assemblyName, string description)
        {
            ...
            try
            {
                ...
                ClrHosting.ClrProcedure method = ClrHosting.GetMethod(clrHostedProcedure, assemblyInformation.RegisteredAssembly);
                CreateObjectEntry(false, VdbObjects.CLRProcedure, Row.EmptyReference, procedureName, Row.EmptyReference, Row.EmptyReference, 0L, true, ClrProcedureCollection.ClrProcedureInformation.CompileSignature(clrHostedProcedure, method.Method.ToString()), description, Encoding.Unicode.GetBytes(assemblyName), null);
        ...
        internal void UnregisterCLRProc(string procedureName, bool commit, IVistaDBClrProcedureInformation clrProc)
        {
            ...
            try
            {
                ...
                DeleteInScope(VdbObjects.CLRProcedure, Row.EmptyReference, procedureName, false);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        internal ClrProcedureCollection LoadClrProcedureObjects()
        {
            LockStorage();
            try
            {
                ClrProcedureCollection procedureCollection = new ClrProcedureCollection();
                TraverseDiskObjects(VdbObjects.CLRProcedure, Row.EmptyReference, new TraverseJobDelegate(LoadClrProceduresJob), procedureCollection, Row.EmptyReference, VdbObjects.CLRProcedure);
        ...
        internal ClrTriggerCollection LoadClrTriggerObjects()
        {
            LockStorage();
            try
            {
                ClrTriggerCollection triggerCollection = new ClrTriggerCollection();
                TraverseDiskObjects(VdbObjects.CLRTrigger, 0UL, new TraverseJobDelegate(LoadClrTriggersJob), triggerCollection, long.MaxValue, VdbObjects.CLRTrigger);
        ...
        internal ClrTriggerCollection LoadClrTriggers(string tableName)
        {
            LockStorage();
            try
            {
                if (tableName == null)
                {
                    ClrTriggerCollection triggerCollection = LoadClrTriggerObjects();
                    TableIdMap tableIdMap = GetTableIdMap();
                    triggerCollection.AssignTableReferences(tableIdMap, tableName);
                    return triggerCollection;
                }
                ulong tableId = GetTableId(tableName, out string description, out Table.TableType type, true);
                ClrTriggerCollection triggers = new ClrTriggerCollection();
        ...
        {
            return PrepareInvoke(procedureName, clrHosting, VdbObjects.CLRProcedure);
        }
        ...
        internal void CreateAssembly(string assemblyName, byte[] coffImage, string description, bool commit)
        {
            ...
            try
            {
                ...
                CreateObjectEntry(false, VdbObjects.Assembly, Row.EmptyReference, assemblyName, Row.EmptyReference, Row.EmptyReference, 0L, true, AssemblyCollection.AssemblyInformation.CompileScriptValue(assembly.FullName, assembly.ImageRuntimeVersion, empty), description, null, coffImage);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        internal void UpdateAssembly(string assemblyName, byte[] coffImage, string description)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    UpdateObjectEntry(false, VdbObjects.Assembly, Row.EmptyReference, assemblyName, Row.EmptyReference, Row.EmptyReference, 0L, true, AssemblyCollection.AssemblyInformation.CompileScriptValue(assembly.FullName, assembly.ImageRuntimeVersion, empty), description, null, coffImage);
                    commit = true;
                }
            }
            catch (Exception ex)
        ...
        internal void DeleteAssembly(string assemblyName, bool force)
        {
            ...
            try
            {
                ...
                if (LocateAssembly(assemblyName, false) == null)
                    throw new VistaDBException(390, assemblyName);
                DeleteInScope(VdbObjects.Assembly, Row.EmptyReference, assemblyName, false);
                commit = true;
            }
            catch (Exception ex)
            {
        ...
        internal void CopyClrProceduresAndTriggersFrom(Database Database)
        {
            ...
            foreach (IVistaDBAssemblyInformation assemblyInformation in assemblyCollection)
            {
                ...
                ImportAssembly(assemblyInformation.Name, ((AssemblyCollection.AssemblyInformation)assemblyInformation).COFFImage, assemblyInformation.FullName, assemblyInformation.ImageRuntimeVersion, vistadbVersion, assemblyInformation.Description);
            }
        ...
        internal void RegisterCLRTrigger(string triggerName, string clrHostedTrigger, string assemblyName, string tableName, TriggerAction eventType, string description)
        {
            ...
            try
            {
                ...
                CreateObjectEntry(false, VdbObjects.CLRTrigger, tableId, triggerName, Row.EmptyReference, Row.EmptyReference, (long)eventType, true, ClrProcedureCollection.ClrProcedureInformation.CompileSignature(clrHostedTrigger, method.Method.ToString()), description, Encoding.Unicode.GetBytes(assemblyName), null);
        ...
        internal void UnregisterCLRTrigger(string triggerName, bool commit, IVistaDBClrTriggerInformation clrTrigger)
        {
            ...
            try
            {
                ...
                DeleteInScope(VdbObjects.CLRTrigger, ((ClrTriggerCollection.ClrTriggerInformation)clrTrigger).ParentTableId, triggerName, false);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        {
            return PrepareInvoke(triggerName, clrHostedTriggers, VdbObjects.CLRTrigger);
        }
        ...
        internal void RegisterStoredProcedure(IStoredProcedureInformation sp)
        {
            ...
            try
            {
                ...
                CreateObjectEntry(false, VdbObjects.StoredProcedure, Row.EmptyReference, sp.Name, Row.EmptyReference, Row.EmptyReference, 0L, true, null, sp.Description, null, sp.Serialize());
                commit = true;
            }
            catch (Exception ex)
            {
        ...
        internal void UnregisterStoredProcedure(string procedureName, bool commit)
        {
            ...
            try
            {
                DeleteInScope(VdbObjects.StoredProcedure, Row.EmptyReference, procedureName, false);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        internal void RegisterUserDefinedFunction(IUserDefinedFunctionInformation udf)
        {
            ...
            try
            {
                ...
                CreateObjectEntry(false, VdbObjects.UDF, Row.EmptyReference, udf.Name, Row.EmptyReference, Row.EmptyReference, udf.ScalarValued ? 0L : 1L, true, null, udf.Description, null, udf.Serialize());
                commit = true;
            }
            catch (Exception ex)
            {
        ...
        internal void UnregisterUserDefinedFunction(string udfName, bool commit)
        {
            ...
            try
            {
                DeleteInScope(VdbObjects.UDF, Row.EmptyReference, udfName, false);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        internal override uint TransactionId
        {
            get
            {
                return ((DatabaseHeader)base.Header).CurrentTransactionId;
            }
        }
        ...
        {
            string name = SyncExtension.TombstoneTablenamePrefix + relatedTableName;
            ulong tableId = GetTableId(name, out string description, out Table.TableType type, false);
        ...
        {
            if ((long)GetTableId(SyncExtension.AnchorTablename, out string description, out Table.TableType type, false) == (long)Row.EmptyReference)
                CreateTable(SyncExtension.GetAnchorSchema(this), true, IsReadOnly, false).Dispose();
            else if (type != Table.TableType.Anchor)
                throw new VistaDBException(213, SyncExtension.AnchorTablename);
        }
        ...
        {
            ITable tbl = OpenClone(SyncExtension.AnchorTablename, IsReadOnly);
            try
            {
                tbl.SetFilter(SyncExtension.SyncTableName + "='" + tableName + "'", true);
                tbl.First();
                if (!tbl.EndOfTable)
                    return;
                tbl.Insert();
                tbl.PutString(SyncExtension.SyncTableName, tableName);
                tbl.Post();
            }
            finally
            {
        ...
        internal void ActivateSyncService(string tableName)
        {
            ...
            try
            {
                ...
                Table.TableSchema newSchema = new Table.TableSchema(SyncExtension.TombstoneTablenamePrefix + tableSchema.Name, Table.TableType.Tombstone, SyncExtension.TombstoneTableDescription + tableSchema.Name, 0UL, this);
        ...
        {
            private VdbObjects id;
            private string name;
            private string description;

            internal DatabaseObject(VdbObjects id, string name, string description)
            {
                this.id = id;
                this.name = name;
            ...
            {
                return EqualNames(name1, name2, true);
            }
            ...
            {
                return EqualNames(name, ((DatabaseObject)obj).Name);
            }
        ...
        private class Descriptors : Dictionary<ulong, Table>
        {
            ...
            internal Table TableInstance(string name, bool readOnly)
            {
                ...
                {
                    if (!table2.IsClosed && DatabaseObject.EqualNames(table2.Name, name) && table2.ModificationVersion >= num && (!table2.IsReadOnly || readOnly))
                    {
                        table1 = table2;
            ...
            internal Table TableInstance(string name)
            {
                ...
                {
                    if (!table.IsClosed && DatabaseObject.EqualNames(table.Name, name))
                        return table;
                }
                return null;
            }
        ...
        private class IdentityCollection : Dictionary<string, List<Row.Column>>
        {
            ...
            internal Row.Column GetLastIdentity(string tableName, string columnName)
            {
                ...
                {
                    if (DatabaseObject.EqualNames(column.Name, columnName))
                        return column;
                }
                return null;
            }
        ...
        internal class ViewList : InsensitiveHashtable, IViewList, IDictionary, ICollection, IEnumerable
        {
            ...
            {
                Add(viewName, new View(viewName, script, scriptExtension, description));
            }

            internal class View : DatabaseObject, IView, IVistaDBDatabaseObject
            {
                private string expression;

                internal View(string name)
                  : base(VdbObjects.View, name, null)
                {
                }

                internal View(string name, string script, byte[] scriptExtension, string description)
                  : base(VdbObjects.View, name, description)
                {
                ...
                {
                    if (base.Equals(obj))
                        return string.Compare(expression, ((View)obj).expression, StringComparison.OrdinalIgnoreCase) == 0;
                    return false;
                }
        ...
        private class EnumCache<T> where T : IEnumerable
        {
            ...
            public bool IsValid(DatabaseHeader header)
            {
                return List != null && (Version.Equals(header.LastVersion) && !(DateTime.Now > ForceRefreshAfter));
            }

            public void Update(DatabaseHeader header, T list)
            {
        ...
        private class EnumKeyedCache<T> : Dictionary<ulong, EnumKeyedCache<T>.Entry> where T : IDictionary
        {
            public bool TryFillFromCache(ulong key, DatabaseHeader header, T list)
            {
            ...
            public bool AddToCache(ulong key, DatabaseHeader header, T list)
            {
                if (ContainsKey(key))
                    this[key] = new Entry(header.LastVersion, list);
                else
                    Add(key, new Entry(header.LastVersion, list));
                return true;
            }
        ...
        internal class StoredProcedureCollection : VistaDBKeyedCollection<string, IStoredProcedureInformation>, IStoredProcedureCollection, IVistaDBKeyedCollection<string, IStoredProcedureInformation>, ICollection<IStoredProcedureInformation>, IEnumerable<IStoredProcedureInformation>, IEnumerable
        {
            ...
            {
                Add(name, new StoredProcedureInformation(name, scriptData, description, status));
            }

            internal class StoredProcedureInformation : DatabaseObject, IStoredProcedureInformation, IVistaDBDatabaseObject
            {
                private string script;

                protected StoredProcedureInformation(VdbObjects type, string name, string script, string description)
                ...
                internal StoredProcedureInformation(string name, byte[] scriptData, string description, bool status)
                  : this(VdbObjects.StoredProcedure, name, Encoding.Unicode.GetString(scriptData, 0, scriptData.Length), description)
                {
                }

                internal StoredProcedureInformation(string name, string script, string description)
                  : this(VdbObjects.StoredProcedure, name, script, description)
                {
                }
        ...
        internal class UdfCollection : VistaDBKeyedCollection<string, IUserDefinedFunctionInformation>, IUserDefinedFunctionCollection, IVistaDBKeyedCollection<string, IUserDefinedFunctionInformation>, ICollection<IUserDefinedFunctionInformation>, IEnumerable<IUserDefinedFunctionInformation>, IEnumerable
        {
            ...
            {
                Add(name, new UserDefinedFunction(name, scriptData, scalarValued, description, status));
            }

            internal class UserDefinedFunction : StoredProcedureCollection.StoredProcedureInformation, IUserDefinedFunctionInformation, IStoredProcedureInformation, IVistaDBDatabaseObject
            {
                ...
                internal UserDefinedFunction(string name, byte[] scriptData, bool scalarValued, string description, bool status)
                  : base(VdbObjects.UDF, name, Encoding.Unicode.GetString(scriptData, 0, scriptData.Length), description)
                {
                ...
                internal UserDefinedFunction(string name, string script, bool scalarValued, string description)
                  : base(VdbObjects.UDF, name, script, description)
                {
                    this.scalarValued = scalarValued;
                }
        ...
        internal class ClrProcedureCollection : VistaDBKeyedCollection<string, IVistaDBClrProcedureInformation>, IVistaDBClrProcedureCollection, IVistaDBKeyedCollection<string, IVistaDBClrProcedureInformation>, ICollection<IVistaDBClrProcedureInformation>, IEnumerable<IVistaDBClrProcedureInformation>, IEnumerable
        {
            ...
            {
                Add(name, new ClrProcedureInformation(name, fullSignature, description, assemblyFullName, status));
            }
            ...
            internal class ClrProcedureInformation : DatabaseObject, IVistaDBClrProcedureInformation, IVistaDBDatabaseObject
            {
                ...
                internal ClrProcedureInformation(string name, string fullSignature, string description, byte[] assemblyName, bool status)
                  : this(VdbObjects.CLRProcedure, name, fullSignature, description, assemblyName, status)
                {
                }

                protected ClrProcedureInformation(VdbObjects type, string name, string fullSignature, string description, byte[] assemblyName, bool status)
                  : base(type, name, description)
                {
                ...
                public override bool Equals(object obj)
                {
                    if (base.Equals(obj) && string.Compare(signature, ((ClrProcedureInformation)obj).signature, StringComparison.OrdinalIgnoreCase) == 0 && ClrHosting.EqualNames(assemblyName, ((ClrProcedureInformation)obj).assemblyName))
                        return string.Compare(fullCLRProcedureName, ((ClrProcedureInformation)obj).fullCLRProcedureName, StringComparison.OrdinalIgnoreCase) == 0;
                    return false;
                }
        ...
        internal class ClrTriggerCollection : VistaDBKeyedCollection<string, IVistaDBClrTriggerInformation>, IVistaDBClrTriggerCollection, IVistaDBKeyedCollection<string, IVistaDBClrTriggerInformation>, ICollection<IVistaDBClrTriggerInformation>, IEnumerable<IVistaDBClrTriggerInformation>, IEnumerable
        {
            ...
            {
                Add(name, new ClrTriggerInformation(name, fullSignature, description, assemblyFullName, status, reference, option));
            }

            internal void AddTrigger(ClrTriggerInformation trigger)
            {
                Add(trigger.Name, new ClrTriggerInformation(trigger.Name, ClrProcedureCollection.ClrProcedureInformation.CompileSignature(trigger.FullHostedName, trigger.Signature), trigger.Description, Encoding.Unicode.GetBytes(trigger.AssemblyName), trigger.Active, trigger.ParentTableId, (long)((IVistaDBClrTriggerInformation)trigger).TriggerAction));
            }

            internal void DropByFullName(string fullName)
            {
                foreach (ClrTriggerInformation triggerInformation in Values)
                {
            ...
            internal void AssignTableReferences(TableIdMap tables, string tableName)
            {
                List<string> stringList = new List<string>();
                foreach (ClrTriggerInformation triggerInformation in Values)
                {
                    string tableNameFromId = tables.GetTableNameFromID(triggerInformation.ParentTableId);
                    if (tableName != null && !DatabaseObject.EqualNames(tableName, tableNameFromId))
            ...
            internal class ClrTriggerInformation : DatabaseObject, IVistaDBClrTriggerInformation, IVistaDBDatabaseObject
            {
                private string signature;
                private string fullCLRProcedureName;
                private bool activeStatus;
                ...
                internal ClrTriggerInformation(string name, string fullSignature, string description, byte[] assemblyName, bool status, ulong reference, long options)
                  : this(VdbObjects.CLRTrigger, name, fullSignature, description, assemblyName, status)
                {
                    this.reference = reference;
                    triggerEvent = options;
                }

                protected ClrTriggerInformation(VdbObjects type, string name, string fullSignature, string description, byte[] assemblyName, bool status)
                  : base(type, name, description)
                {
                    DecompileSignature(fullSignature);
                    this.assemblyName = Encoding.Unicode.GetString(assemblyName, 0, assemblyName.Length);
                ...
                public override bool Equals(object obj)
                {
                    if (base.Equals(obj) && string.Compare(signature, ((ClrTriggerInformation)obj).signature, StringComparison.OrdinalIgnoreCase) == 0 && ClrHosting.EqualNames(assemblyName, ((ClrTriggerInformation)obj).assemblyName))
                        return string.Compare(fullCLRProcedureName, ((ClrTriggerInformation)obj).fullCLRProcedureName, StringComparison.OrdinalIgnoreCase) == 0;
                    return false;
                }
        ...
        internal class AssemblyCollection : VistaDBKeyedCollection<string, IVistaDBAssemblyInformation>, IVistaDBAssemblyCollection, IVistaDBKeyedCollection<string, IVistaDBAssemblyInformation>, ICollection<IVistaDBAssemblyInformation>, IEnumerable<IVistaDBAssemblyInformation>, IEnumerable
        {
            ...
            {
                Add(assemblyName, new AssemblyInformation(assemblyName, assemblyBody, instantiateAssemblies, script, description, parentConnection));
            }

            internal class AssemblyInformation : DatabaseObject, IVistaDBAssemblyInformation, IVistaDBDatabaseObject
            {
                ...
                internal AssemblyInformation(string name, byte[] assemblyBody, bool instantiate, string scriptValue, string description, DirectConnection parentConnection)
                  : base(VdbObjects.Assembly, name, description)
                {
                ...
                public override bool Equals(object obj)
                {
                    if (base.Equals(obj) && string.Compare(fullName, ((AssemblyInformation)obj).fullName, StringComparison.OrdinalIgnoreCase) == 0)
                        return string.Compare(runtimeVersion, ((AssemblyInformation)obj).runtimeVersion, StringComparison.OrdinalIgnoreCase) == 0;
                    return false;
                }
        ...
        internal class RelationshipCollection : VistaDBKeyedCollection<string, IVistaDBRelationshipInformation>, IVistaDBRelationshipCollection, IVistaDBKeyedCollection<string, IVistaDBRelationshipInformation>, ICollection<IVistaDBRelationshipInformation>, IEnumerable<IVistaDBRelationshipInformation>, IEnumerable
        {
            ...
            internal class RelationshipInformation : DatabaseObject, IVistaDBRelationshipInformation, IVistaDBDatabaseObject
            {
                ...
                internal RelationshipInformation(string name, ulong foreignTableId, string foreignKey, ulong primaryTableId, int options, string description)
                  : base(VdbObjects.Relationship, name, description)
                {
                ...
                public override bool Equals(object obj)
                {
                    if (base.Equals(obj) && options == ((RelationshipInformation)obj).options && ((long)foreignTableId == (long)((RelationshipInformation)obj).foreignTableId && (long)primaryTableId == (long)((RelationshipInformation)obj).primaryTableId))
                        return string.Compare(foreignKey, ((RelationshipInformation)obj).foreignKey, StringComparison.OrdinalIgnoreCase) == 0;
                    return false;
                }
        ...
        internal class DatabaseHeader : ClusteredRowsetHeader
        {
            ...
            new internal static DatabaseHeader CreateInstance(DataStorage parentDatabase, int pageSize, CultureInfo culture)
            {
                return new DatabaseHeader(parentDatabase, HeaderId.DATABASE_HEADER, Type.Unique, pageSize, culture);
            }

            protected DatabaseHeader(DataStorage parentDatabase, HeaderId id, Type type, int pageSize, CultureInfo culture)
              : base(parentDatabase, id, type, pageSize, culture)
            {
                ...
                AppendColumn(new CharColumn(FileCopyrightString, 120, 1252, CultureInfo.InvariantCulture, false));
            ...
            protected override Row OnAllocateDefaultRow(Row rowInstance)
            {
                ...
                for (; num1 < ColumnCount; ++num1)
                {
                    int index1 = num2;
                    int num3 = index1 + 1;
                    Column column1 = this[index1];
                    int index2 = num3;
                    int num4 = index2 + 1;
                    Column column2 = this[index2];
                    int index3 = num4;
                    int num5 = index3 + 1;
                    Column column3 = this[index3];
                    int index4 = num5;
                    int num6 = index4 + 1;
                    Column column4 = this[index4];
                    int index5 = num6;
                    int num7 = index5 + 1;
                    Column column5 = this[index5];
                    int index6 = num7;
                    int num8 = index6 + 1;
                    Column column6 = this[index6];
                    int index7 = num8;
                    int num9 = index7 + 1;
                    Column column7 = this[index7];
                    int index8 = num9;
                    int num10 = index8 + 1;
                    Column column8 = this[index8];
                    bool syncColumn = ((column8.IsNull ? 0L : (long)column8.Value) & 1L) == 1L;
                    int index9 = num10;
                    num2 = index9 + 1;
                    Column column9 = this[index9];
                    Column emptyColumnInstance = ParentStorage.CreateEmptyColumnInstance((VistaDBType)(byte)column1.Value, (short)column2.Value, (int)column3.Value, true, syncColumn);
            ...
            private static DatabaseHeader CreateVdb3Instance(DatabaseHeader header)
            {
                header = new DatabaseHeader(header.ParentStorage, (HeaderId)header.RowId, (Type)header.Signature, header.PageSize, header.Culture);
                header.IsVdb3Header = true;
                Column column1 = header[header.encryptionKeyMd5HeaderPosition];
                Column column2 = header[header.encryptionKeyMd5HeaderPosition + 1];
                Column column3 = header[header.databaseFileSchemaVersionHeaderPosition];
                Column column4 = header[header.NumberColumnsHeaderPosition];
                Column column5 = header[header.freeSpaceMapLocationHeaderPosition];
                Column column6 = header[header.freeSpaceMapLocationHeaderPosition + 1];
                Column column7 = header[header.schemaVersionGuidHeaderPosition];
                Column column8 = header[header.schemaVersionGuidHeaderPosition + 1];
                Column column9 = header[header.transactionIdHeaderPosition];
            ...
            private void OnActivateError(ulong position, Exception ex, ref bool handled)
            {
                ...
                ParentStorage.SwapHeader(CreateVdb3Instance(this));
